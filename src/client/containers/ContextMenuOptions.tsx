...
import { ArrowUp, Download, Star, Trash, X, Edit2, Clipboard, Italic } from 'react-feather'
...
const NotesOptions: React.FC<NotesOptionsProps> = ({ clickedNote }) => {
  ...
  const _calmTheNote = (noteId: string) => dispatch(calmTheNote({ noteId }))
  ...
  const dontPanicHandler = (e: React.SyntheticEvent, note: NoteItem) => {
    e.preventDefault()

    _calmTheNote(note.id)
  }
